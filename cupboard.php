...
$one = "One Piece Cupboard";
$two = "Two Piece Cupboard";
$three = "Three Piece Cupboard";
?>

<style>
<?php include 'css/cupboardstyle.css'; ?>
...
<body>
    ...
    <div class="cup">
        <img src="image/cupboard2.jpg">
        <p>"Made-to-measure wardrobe cabinets from Pickawood fit exactly to your needs.<br> Easily create your dream wardrobe or get inspired by existing designs."</p>
        <br><br><br><br><br><br>

        <i class="fa-solid fa-angle-right"></i>
        <b>High-quality wardrobe cabinets,</b><br>
        <i class="fa-solid fa-angle-right"></i>
        <b>Craftsmanship with exact measurements</b>
    </div>

    

    <div class="container mt-5" style="margin-top:40px;">
    <h3  class="text-center"> <?php echo $one; ?></h3>

    <div class="row">
      <?php
      $single_ = mysqli_query($conn, "SELECT * FROM `cupboard_tbl` WHERE Type = '$one'");
      ?>
      <?php
            ...
            <div class="card-img">
              ...
              <div class="overlay">
                <div class="overlay-content">
                  <?php  if ($row['Stock']> 0){?><a class="btn btn-warning" href="order.php?id=<?php echo $row['id'].'&cupboardtype='.  $row['Type'];?>">Order Now</a> <?php }
                   elseif($row['Stock']<= 0)
    ...
    <div class="container">
      <h3 class="text-center"> <?php echo $two; ?></h3>
      <div class="row">
        <?php
        $single_ = mysqli_query($conn, "SELECT * FROM `cupboard_tbl` WHERE Type = '$two'");
        ?>
        <?php
        while ($row = mysqli_fetch_assoc($single_)) {
        ?>
  ...
  <div class="container">
    <h3 class="text-center"> <?php echo $three; ?></h3>
    <div class="row">
      <?php
      $single_ = mysqli_query($conn, "SELECT * FROM `cupboard_tbl` WHERE Type = '$three'");
      ?>
      <?php
      while ($row = mysqli_fetch_assoc($single_)) {
      ?>
